...
BASE_INSTRUCTIONS = (
    ...
    "Before EVERY call to `generative_generate_prefab_ui`, ALWAYS call "
    "`generative_search_prefab_components` first to discover what's currently available — "
    "the library evolves and richer components ship regularly. Build with the components "
    "discovery returns; favour MORE components rather than fewer for an information-dense "
    "report.\n\n"
    "Layout: always use `Dashboard` as the outermost container with `DashboardItem` "
    "children for placement.\n\n"
    "## Map requests — use `show_map`, NOT Prefab UI\n\n"
    "When the user asks to see locations / a map / a route / charging stops on a map, "
    "call the `show_map` tool directly with a list of addresses. Do NOT search Prefab "
    "components for a Map widget — there is none. Typical triggers:\n"
    "- \"map my last trip / drive\" → pass `start_address` and `end_address` of the drive\n"
    "- \"map my charging stops\"   → pass each charge's `address`\n"
    "- \"where is X / show me on a map\" → pass the address or place name as a single item"
)
